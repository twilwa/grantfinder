...
- [ ] 2.1 Add an `isPlatformAdministrator` predicate over the verified
  `privyUserId` against `PLATFORM_ADMIN_PRIVY_IDS`, wired through the existing
  `authenticate` path without changing the role model
- [ ] 2.2 Add admin-only REST routes and JSON-RPC methods to create, list, update,
...
- [ ] 3.1 Implement most-specific-wins evaluation (user → organization → role →
  default) in the service layer with focused precedence tests
- [ ] 3.2 Extend the `GET /api/auth/session` payload with the resolved
...
- [ ] 4.1 Thread `enabledFeatures` from the session payload into the client and
  gate flagged sidebar tabs / `SectionCard`s on feature-key membership
- [ ] 4.2 Add an administrator-only flag management surface (list/create/update/
